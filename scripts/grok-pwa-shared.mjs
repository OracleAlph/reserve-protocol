...
export function grokOgHeadTags({
  ...
  const canonical = String(site.url ?? "").trim().replace(/\/+$/, "");
  const origin = canonical || (publicHost ? `https://${publicHost}` : "");
  if (canonical) {
  ...
  if (origin) {
    ...
    let image = custom
      ...
      : `${ogServiceUrl()}/v1/card.png?host=${encodeURIComponent(publicHost || new URL(origin).host)}&title=${encodeURIComponent(title)}`;
